...
class RescaleTransform:
    ...
    def __call__(self, image):
        ...
        i=0
        for x in image_copy:      
            x = (((x - self._data_min) * (self.max - self.min)) / (self._data_max - self._data_min)) + self.min
            image_copy[i]=x
            i=i+1
        ret_image = image_copy
        ########################################################################
        # TODO:                                                                #
...
class NormalizeTransform:
    ...
    def __call__(self, images):
        ...
        if isinstance(mean, (int, float)):
            ...
            if len(mean.shape) == 1 and mean.shape[0] == images.shape[-1]:
                images = (images - mean) / std
            else:
                raise ValueError("Mean should be a single value or a 1D numpy array of size C.")
        else:
            raise ValueError("Mean should be a single value or a 1D numpy array of size C.")
