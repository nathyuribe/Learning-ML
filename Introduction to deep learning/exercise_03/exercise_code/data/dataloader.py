    ...
    def __iter__(self):
        ...
        for index in index_iterator:  # iterate over indices using the iterator
            ...
            if len(batch) == self.batch_size:
                ...
                #batch=self.combine_batch_dicts(batch)
                batch_dict = {}
                ...
                #batch=self.batch_to_numpy(batch_dict)
                numpy_batch = {}
        ...
        #If there is elements on the last batch but it's lenght is not equal to batch length 
        # and drop last is False  we still yield the batch      
        if  bool(batch) and self.drop_last==False: 
            batch_dict = {}
            for data_dict in batch:
                for key, value in data_dict.items():
                    if key not in batch_dict:
