...
class BCE2(Loss):
    ...
    def forward(self, y_out, y_truth, reduction='mean'):
        ...
        :param reduction: str, specifies the reduction to apply to the output.
                          'none' | 'mean' | 'sum'. 'mean': the sum of the output will be divided by the number of elements in the output, 
                          'sum': the output will be summed, 'none': no reduction will be applied.
        :return: Scalar value of binary cross-entropy loss.
        """

        # Ensure that y_out is within a valid range to avoid log(0)
        y_out = np.clip(y_out, 1e-10, 1 - 1e-10)
    ...
    def backward(self, y_out, y_truth):
        ...
        # Ensure that y_out is within a valid range to avoid division by zero
        y_out = np.clip(y_out, 1e-10, 1 - 1e-10)
